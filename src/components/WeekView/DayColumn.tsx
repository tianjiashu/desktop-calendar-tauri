// ========== Day column component (F10, Phase F: empty state + skeleton) ==========
...
interface DayColumnProps {
  ...
  /** Called on double-click empty area and opens create dialog with preselected hour-slot range. */
...
function calcHourSlot(clientY: number, containerTop: number): HourSlot | null {
...
const DayColumn: React.FC<DayColumnProps> = ({
  ...
  const [hoverSlot, setHoverSlot] = useState<HourSlot | null>(null);
  ...
  const dayEvents = events.filter(event => (
    event.start_time < dayEnd.getTime() && event.end_time > dayStart.getTime()
  ));

  const visibleEvents = filterVisibleEvents(dayEvents);
  const layoutEvents: EventWithLayout[] = assignColumns(visibleEvents);
  const conflictIds = getConflictIds(layoutEvents, date);
  ...
  const handleMouseMove = useCallback((e: React.MouseEvent) => {
    ...
    setHoverSlot(calcHourSlot(e.clientY, rect.top));
  }, []);
  ...
  return (
    <div className={`day-column ${isToday ? 'today' : ''}`}>
      <div ref={headerRef as React.Ref<HTMLDivElement> | undefined}>
        <DayHeader
          date={date}
          isToday={isToday}
      ...
      >
        ...
        {isLoading && layoutEvents.length === 0 && (
          <div className="skeleton-container">
            {[45, 30, 55, 40, 25].map((pct, i) => (
              <div
                key={pct}
...
function getConflictIds(layoutEvents: EventWithLayout[], date: Date): Set<string> {
    ...
    for (let j = i + 1; j < layoutEvents.length; j++) {
      ...
      if (a.start_time < b.end_time && a.end_time > b.start_time) {
        ...
        logger.info(
          ...
          `A=${a.title}(${a.id.slice(0, 4)}) [${formatEventTimeRange(a)}] vs ` +
          `B=${b.title}(${b.id.slice(0, 4)}) [${formatEventTimeRange(b)}]`,
        );
      }
    }
  }
...
function formatEventTimeRange(event: CalendarEvent): string {
  const start = new Date(event.start_time).toLocaleTimeString('zh-CN', { hour: '2-digit', minute: '2-digit' });
  const end = new Date(event.end_time).toLocaleTimeString('zh-CN', { hour: '2-digit', minute: '2-digit' });
  return `${start}-${end}`;
}
